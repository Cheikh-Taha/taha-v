        ...
        {appointments.map((item,index) =>(
          <div key={index} className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' >
            ...
            <div className='flex flex-col justify-end gap-2'>
              ...
              <button onClick={()=>cancelAppointment(item._id)} className='text-sm text-stone-500 text-center sm:min-w-48 py-2 border rounded hover:bg-red-600 hover:text-white transition-all duration-150 cursor-pointer'>cancel Appointment</button>
            </div>
          </div>
        ))}
      </div>
